...
public class Ex04 {
	// Stream(스트림) : 데이터를 목적지까지 입/출력 하기 위한 방법
	...
	//				 영문자, 대소문자, 숫자를 스트림 할 수 있다. (영어외의 문자(한글, 한자 등)은 불가능)
	//		- 최상위 클래스 : InputStream(입력), OutputStream(출력)
	//																	대부분은 byte스트림을 사용!
	
	// 2. 문자 스트림(사람중심) : 모든 처리를 2byte씩 처리
	//		- 대상 : 전 세계의 모든 언어로 구성된 문서 파일(byte스트림도 되지만 문자용으로 쓰기 편하려고 만듬)
	...
	// 4. 오브젝트 스트림 : 객체를 직렬화 한 후 객체 단위로 입/출력한다.(객체 직렬화)                 
