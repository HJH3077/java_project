...
public abstract class Ex01 { // *** abstract를 쓰려면 클래스에도 붙어야한다! ***
	// 추상 클래스 : 추상메소드를 가지고 있는 클래스를 추상클래스라고 한다.
	//				 일반적인 메소드와 필드를 가질 수 있다.
	//				 추상클래스는 abstract 예약어를 사용한다.
	
	// 추상 메소드 : 구체화가 되지 않은 메소드를 의미한다. (body 블록이 존재하지 않는 메소드를 말함.)
	//				 body 블록이 없으면 실행문도 없다.(= 구체화가 되지 않았다는 뜻)
	//				 추상메소드에는 반드시 abstract 예약어를 사용한다.
	
	// 다형성 : 하나의 메소드나 클래스가 있을 때 이것들을 다양한 방법으로 동작시키는 것을 의미한다.
	//			부모가 추상으로 한개의 추상메소드를 만들어 두면 자식들이 가져가서 각자의 다른 기능을 추가
	int s1 = 10;				// 인스턴스 변수
	...
	// 추상메소드
	public abstract void sound(); // body({ } 부분)가 없음
