...
class Ex02 
{
	...
	// for문을 만나면 무조건 초기식으로 이동 ->	초기식에서는 조건식으로 이동 -> 조건식이 참이면 for문을 실행
	// -> 실행 중 for문의 끝을 만나면 무조건 증감식으로 -> 증감식이 끝나면 다시 조건식으로 이동 -> 조건식이 거짓이면 for문의 밖으로 나감.
	...
	// 0 ~ 10까지 짝수만 출력  // for(int i = 0; i <= 10; i = i + 2) 인 방법도 가능
	for (int i = 0; i<= 10; i++)
	{
		if(i % 2 == 0)
			System.out.print(i + " ");
