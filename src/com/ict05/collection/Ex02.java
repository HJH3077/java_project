...
public class Ex02 {
	public static void main(String[] args) {
		...
		//		그러나 TreeSet은 내부에서 항상 오름 차순 정렬상태를 유지한다.
		//		set은 특정 기준으로 정렬을 할 수 없다. (무조건 이름 기준 오름차순)
		
		// 컬렉션 생성
		// HashSet<제네닉타입 = 객체타입 = 같은클래스> 참조변수 = new HashSet<제네닉타입>();
		// HashSet<제네닉타입 = 객체타입 = 같은클래스> 참조변수 = new HashSet(); // 자바버전 높으면 <제네닉타입 생략가능>
		...
		// 방법2) 기본자료형을 그냥 넣는다.
		//		  기본자료형이 자동으로 객체가 되어서(AutoBoxing) 넣어지.
		...
		// 하나씩 꺼내서 사용하기!
		// 1. 개선된 for문 (그냥 for문 사용 불가! why? index가 없어서)
		for (String k : h1) { // Set은 막 집어 넣는거가 때문에 순서가없음.
			String msg = k; 
		...
		// 2. iterator()
		Iterator<String> it = h1.iterator();					// [1, 3, 5]면 1(자기) 3(다음)
		while (it.hasNext()); // hasNext가 true라서 들어감 hasNext() : 자기 다음 객체가 존재하면 true, 존재하지 않으면 false
		String msg = (String) it.next(); // next() : 다음 개체를 return하고 다음 객체자리로 이동 즉, 1의 인덱스인 내가 3을 꺼내고 3자리에감
		System.out.println(msg);
	}	
}
